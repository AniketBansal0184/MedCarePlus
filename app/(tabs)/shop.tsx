// import { View, Text, FlatList, Image, StyleSheet, TouchableOpacity, ActivityIndicator, TextInput, Platform,Alert, StatusBar } from 'react-native';
...
//     textShadow: '0px 1px 6px rgba(37, 99, 235, 0.2)',
//   },
//   searchWrap: {
//     flexDirection: 'row',
//     alignItems: 'center',
...
export default function Shop() {
  ...
  const debouncedSetSearch = useCallback(debounce(text => setSearch(text.trim()), 400), []);
  ...
  useEffect(() => {
    ...
    // Always include all internal products
    let internal = PRODUCTS;
    if (clean) {
      internal = PRODUCTS.filter(p => p.name.toLowerCase().includes(clean));
    }

    if (!clean) {
      setFiltered(internal);
      setLoading(false);
      return;
    }

    // Fetch external data from FDA API
    axios.get(`https://api.fda.gov/drug/ndc.json?search=brand_name:"${encodeURIComponent(clean)}"+OR+generic_name:"${encodeURIComponent(clean)}"&limit=10`)
      .then(async res => {
        const results = res.data.results || [];
        const externalMapped = await Promise.all(results.map(async (item, idx) => {
          const name = (item.brand_name || item.generic_name || '').trim();
          return {
            id: item.product_ndc || `fda_${idx}_${Date.now()}`,
            name,
            mrp: 100 + idx * 25, 
            category: item.dosage_form || 'External',
          };
        }));

        // Merge internal and external results, ensuring no duplicates by ID
        const finalList = [...internal];
        externalMapped.forEach(externalItem => {
          if (!finalList.some(i => i.id === externalItem.id)) {
            finalList.push(externalItem);
          }
        });
      ...
      .catch(err => {
        //console.error('FDA fetch error:', err.response?.data || err.message);
        setFiltered(internal); // Fallback to internal results
      })
  ...
  const addToCart = async (item) => {
    ...
    const newCart = [...localCart];
    const idx = newCart.findIndex(x => x.name === item.name);
  ...
  const renderItem = ({ item, index }) => (
    ...
    >
      ...
      <Text style={styles.name}>{item.name}</Text>
      <View style={styles.priceWrap}>
        <Text style={styles.price}>₹{item.mrp || item.price}</Text>
        <Text style={styles.cat}>{item.category}</Text>
      </View>
      <TouchableOpacity onPress={() => addToCart(item)} style={styles.btn}>
        <LinearGradient colors={['#60a5fa', '#2563eb']} style={styles.btnGradient}>
          <Text style={styles.btnText}>{addedItemId === item.id ? 'Added!' : 'Add to Cart'}</Text>
        </LinearGradient>
      </TouchableOpacity>
    </MotiView>
  );
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#f8fafc', padding: 16, paddingTop: Platform.OS === 'ios' ? 50 : StatusBar.currentHeight + 20 },
  title: { fontSize: 28, fontWeight: '800', color: '#0f172a', textAlign: 'center', marginBottom: 20 },
  searchWrap: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 16, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 20, elevation: 4 },
  searchInput: { flex: 1, fontSize: 16, color: '#0f172a' },
  card: { backgroundColor: '#fff', borderRadius: 16, padding: 12, marginBottom: 12, marginHorizontal: 4, alignItems: 'center', width: '48%', elevation: 5 },
  img: { width: 64, height: 64, borderRadius: 12, marginBottom: 10 },
  name: { fontSize: 14, fontWeight: '600', color: '#0f172a', textAlign: 'center', marginBottom: 6 },
  priceWrap: { width: '100%', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
  price: { fontSize: 15, fontWeight: '700', color: '#16a34a' },
  cat: { fontSize: 10, fontWeight: '600', color: '#2563eb', backgroundColor: '#eef4ff', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 },
  btn: { width: '100%' },
  btnGradient: { paddingVertical: 10, borderRadius: 12, alignItems: 'center', elevation: 3 },
  btnText: { color: '#fff', fontSize: 13, fontWeight: '700' },
  emptyText: { textAlign: 'center', fontSize: 16, color: '#64748b', marginTop: 20 },
});